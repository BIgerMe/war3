...
class imgController extends baseController
{
    ...
    public function personalLists(){
        $lists  = $this->attachmentModel->where(['user_id'=>$this->getUserId()])->order('create_at desc')->select();
        Json(0,$lists);
    }
    /*图片新增*/
    public function addAttachment(){
        ...
        $exist = $this->attachmentModel->where(['path'=>$path,'user_id'=>$this->getUserId()])->select();
